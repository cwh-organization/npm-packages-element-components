...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const { hasChild, showAddBtn, add, del, upChildData } = useFormChild(props, emit);
    ...
    function createDefault() {
      ...
      if (hasChild.value) {
        if (isArray(currentValue)) {
          const child = (currentValue as UnknownObject[]).map((value, index) => {
                ...
                (item.value.children as IFormColumns).map(child => {
                  return h(
                    ProFormItem,
                    {
                      ...
                      'onUpdate:modelValue': (childValue: UnknownObject) => upChildData(childValue, index),
                    },
                    slots,
                  );
                }),
        ...
        list.push(
          h(ProFormComponent, {
            ...item.value.props,
            is: item.value.component as unknown as any,
