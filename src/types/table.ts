...
import type { UnknownFunction, IPlacementType, StringObject, DeepKeyof, MaybeArray, ExternalParam, IColumnEnum } from './index';
...
export interface TableCommonColumn<T = ExternalParam> extends StringObject, TableColumnsProps {
  /** column label */
  label?: string;
  /** column width */
  width?: string;
...
export interface ITableIndexColumns extends TableCommonColumn {
  ...
  index?: number | ((index: number, currentPage: number, pageSize: number) => number | string);
}

/** Table Selection Columns Options */
export interface ITableSelectionColumns<T = ExternalParam> extends TableCommonColumn {
