import { ComputedRef, computed, Ref, unref, shallowRef, inject } from 'vue';
import { useProOptions } from './index';
import { filterDeep, isObject, objectDeepMerge } from '../utils/index';
import type { ITableColumns, TableColumnsProps, ITableExpose, IPagination, UnknownObject, StringObject, MaybeRef, MaybeArray } from '../types/index';

export function useTableColumns(props: Readonly<{ columns?: ITableColumns }>): ComputedRef<ITableColumns> {
  return computed<ITableColumns>(() => {
    return filterDeep(props.columns || [], 'hide', false);
  });
}

export function useTableDefaultBind(props: Readonly<TableColumnsProps>): ComputedRef<TableColumnsProps> {
  return computed(() => ({
...
export function useTableBind<T extends ColumnsBind>(
  ...
  return computed(() => {
    ...
    const _option = isObject(_currentBind) ? { showOverflowTooltip: true, ..._currentBind } : undefined;
...
export function usePagination(
  props: Readonly<{ pagination?: IPagination | false; request?: (...args: any[]) => Promise<{ total: number; data: Array<any> }> }>,
  emit: (
    event: 'update:currentPage' | 'update:pageSize' | 'size-change' | 'current-change' | 'prev-click' | 'next-click',
    ...args: unknown[]
  ) => void,
): {
  ...
  const pagination = computed(() => {
    ...
    return props.pagination ? objectDeepMerge<IPagination>(options.pagination, props.pagination) : options.pagination;
  });
