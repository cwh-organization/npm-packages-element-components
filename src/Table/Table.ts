import { defineComponent, h, toRefs, VNode, provide, ref, reactive } from 'vue';
import { ElTable, ElTableColumn, ElPagination } from 'element-plus';
import { useTableColumns, useTableBind, useTableDefaultBind, useTableMethods, usePagination } from '../composables';
import props from './props';
import ProTableItem from './TableItem';
import type { ITableSelectionColumns, ITableExpandColumns, ITableIndexColumns, ITableMenuColumns, StringObject } from '../types/index';
import { isFunction } from '@vue/shared';

export default defineComponent({
  name: 'ProTable',
  props,
  emits: ['update:currentPage', 'update:pageSize', 'size-change', 'current-change', 'prev-click', 'next-click'],
  setup(props, { slots, attrs, emit, expose }) {
    ...
    const bindSelection = useTableBind<ITableSelectionColumns>(selection, defaultBind);
    ...
    function createColumn() {
      ...
      if (selection.value) {
        list.push(h(ElTableColumn, { type: 'selection', ...bindSelection.value }));
      }
      ...
      if (index.value) {
        const { index, ...rest } = bindIndex.value;
        if (isFunction(index)) {
          list.push(h(ElTableColumn, { type: 'index', index: idx => index(idx, props.currentPage, props.pageSize), ...rest }));
        } else {
      ...
      if (menu.value != null && menu.value !== false && slots.menu) {
        list.push(
          h(
            ElTableColumn,
            { type: 'menu', ...bindMenu.value },
            {
              default: (scope: StringObject) => slots.menu && slots.menu({ ...scope, size: props.size }),
            },
          ),
        );
      }
